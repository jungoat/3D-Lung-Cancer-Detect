...
from build import build2dLungMask
from p2ch13.dsets import Ct, TrainingLuna2dSegmentationDataset
from p2ch13.model import SegmentationMask, MaskTuple


clim=(-1000.0, 300)
class LunaDataset(TrainingLuna2dSegmentationDataset):
    def __init__(self, sortby_str='label_and_size', series_uid=None):
        super().__init__(series_uid=series_uid)
        if sortby_str == 'label_and_size':
            self.candidateInfo_list.sort(
                key=lambda x: (not x.isNodule_bool, -x.diameter_mm)
            )
