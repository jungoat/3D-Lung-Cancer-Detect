...
class MalignantLunaDataset(LunaDataset):
    def __init__(self, ratio_int=0, **kwargs):
        super().__init__(**kwargs)
        self.ratio_int = ratio_int

        # 전체 후보 불러오기
        all_candidates = getCandidateInfoList()
        cand_dict = {
            (c.series_uid, tuple(round(x, 1) for x in c.center_xyz)): c
            for c in all_candidates if c.isNodule_bool
        }

        self.ben_list = []
        self.mal_list = []

        print("[pylidc] Loading malignancy info...")
        ...
        for scan in pl.query(pl.Scan):
            ...
            for ann in scan.annotations:
                malignancy_score = ann.malignancy
                if not (1 <= malignancy_score <= 5):
                    continue

                # 좌표를 key로 사용
                center_xyz = ann.centroid
                rounded_xyz = tuple(round(x, 1) for x in center_xyz)
                key = (uid, rounded_xyz)

                if key in cand_dict:
                    c = cand_dict[key]
                    new_cand = CandidateInfoTuple(
                        isNodule_bool=True,
                        hasAnnotation_bool=True,
                        isMal_bool=(malignancy_score >= 4),
                        diameter_mm=c.diameter_mm,
                        series_uid=c.series_uid,
                        center_xyz=c.center_xyz,
                    )

                    if new_cand.isMal_bool:
                        self.mal_list.append(new_cand)
                    else:
                        self.ben_list.append(new_cand)

        # 음성 후보까지 포함
        self.neg_list = [c for c in all_candidates if not c.isNodule_bool]

        print(f"[pylidc] Loaded benign: {len(self.ben_list)}, malignant: {len(self.mal_list)}, negative: {len(self.neg_list)}")
